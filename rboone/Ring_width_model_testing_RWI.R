...
model{
  ...
  for(c in 1:Ncores){
    for(y in 2:Nyears){
      ...
      mu.LogRWI[y,c] <- a[1,c] + a[2,c]*antX[y,1] + 
        a[3,c]*antX[y,2] + a[4,c]*antX[y,1]*antX[y,2] + 
    ...
    for(t in 1:Nlag){
      ...
      for(m in 1:12){
        ...
        for(i in 1:Nyears){
          ...
          antX1[i,m,t,v] <- weight[m,t,v]*(equals(v,1)*ppt[(i+4)-t+1,12*(site-1) + m] + 
