# This is a copy of the tree ring memory model with alterations to make it run better for the SAM Dead-Live data.
# The age effect will be removed and the computational "tricks" will be implemented.
# Run this on sites with many individuals for the best results maybe.

# Tree-ring memory model
# data files for Pinus Edulis (PIED) growing near Montrose, CO:
# ring RWIs (mm): BUGS_data_RWIs_Montrose_PIED
...
model{
  ...
  # example application.
  #RWI.matrix[yrID[i],CoreID[i]] <- RWI[i]
  #age[yrID[i],CoreID[i]] <- Age[i]
  #}
  # for(y in 1:Nyears){
  # 	for(c in 1:Ncores){
  # 		# log transform ring RWI (mm) data; because there are some missing rings
  # 		# (RWI = 0), add one to RWI value:
  # 		LogRWI[y,c] <- log(RWI.matrix[y,c]+1)
  # 		}
  # Data not used in ring RWI and climate data. If using newer versions of OpenBUGS,
  ...
  # ring-RWIs are modeled as function of previous ring-RWI, so only
  # define likelihood for 2nd year of observation and beyond):
  for(c in 1:Ncores){
    for(y in 2:Nyears){
      # Likelihood for log ring-RWI data:			
      LogRWI[y,c] ~ dnorm(mu.LogRWI[y,c], tau)
      # Replicated data for evaluating model fit:
      LogRWI.rep[y,c] ~ dnorm(mu.LogRWI[y,c], tau)
      # Mean model that incorporates age effect, antecedent climate effects (antX), and
      # previous ring-RWI, which is centered around a value that is close to the 
      # observed mean ring RWI of 0.5:
      #covariate centering: mean age = 50
      mu.LogRWI[y,c] <- a[1,c] + a[2,c]*antX[y,1,status[c]] + 
        a[3,c]*antX[y,2,status[c]] + a[4,c]*antX[y,1,status[c]]*antX[y,2,status[c]] + 
        a[5,c]*(AR1[y,c] - log(1+1))
      
      #data likelihood
      # age[y, c]~dnorm(0,0.0001)T(0,)#C(0,)  I(0,)
      AR1[y, c]~dnorm(1, tau)T(0,)#check with somebody about this please
    }
  }
    ...
    for(v in 1:Nv){
      ...
      for(t in 1:Nlag){
        ...
        for(m in 1:12){
          ...
          delta[m,t,v,s] <- (deltaX[block[t,m],v,s]/BlockSize[t])*(1-equals(t,1)*step(m-9.5))
          ...
          # (ppt and Tave) begin at year 1901 (9 years earlier than ring RWIs, thus, Nlag can't be
          # greater than 9).
          for(i in 1:Nyears){
            ...
            antX1[i,m,t,v,s] <- weight[m,t,v,s]*(equals(v,1)*ppt[(i+4)-t+1,12*(site-1) + m] + 
                                               equals(v,2)*tave[(i+4)-t+1,12*(site-1) + m])
          }	
        }
      ...
      for(i in 1:Nyears){
        ...
        antX[i,v,s] <- ant.sum2[i,v,s] - xave[v]
      }
    } 
