...
model{
	...
	for(r in 1: Nring){
			...
			mu.LogWidth[r] <- a[1,SiteID[r]] + a[2,SiteID[r]]*antX[Year[r],1,SiteID[r]] + 
						a[3,SiteID[r]]*antX[Year[r],2, SiteID[r]] + a[4,SiteID[r]]*antX[Year[r],1, SiteID[r]]*antX[Year[r],2, SiteID[r]] + 
						a[5,SiteID[r]]*(AR1[r] - log(1+1))
			
			#data likelihood
			AR1[r]~dnorm(1, tau) #check with somebody about this please
	}
  ...
  for(v in 1:Nv){
    ...
    for(t in 1:Nlag){
      ...
      for(m in 1:12){
        ...
        delta[m,t,v] <- (deltaX[block[m,t],v]/BlockSize[t])*(1-equals(t,1)*step(m-9.5))
